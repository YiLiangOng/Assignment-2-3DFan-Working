...
This is a 3D fan with a base, pole, hub, and a selectable three, four, or
five blades. It is made out of one shared cube. It works on Android, Windows,
and WebGL using OpenGL ES 3.0. The C++ code is shared in the `Scene/` folder.

- Tap / click: toggle the fan **ON** / **OFF** (logged as `Fan ON` / `Fan OFF`)
- Long press / click-and-hold for 0.55 seconds: cycle **3 -> 4 -> 5** blades
  (logged as `Blade count: N`)
- Drag / swipe: spin the fan faster, proportional to swipe speed; after release
  the added speed coasts down smoothly to the base speed

See `Scene/Fan.h` and `Scene/Fan.cpp` for the implementation and a full
explanation of the part hierarchy and gesture logic.

## Project layout![[Pasted image 20260711170004.png]]
...
click-and-hold to change the blade count, click-drag to spin it faster, `ESC`
to quit.
...
the fan, click-and-hold to change the blade count, click-drag to spin it
faster.
